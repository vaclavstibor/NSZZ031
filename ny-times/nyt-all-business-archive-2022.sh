...
for month in {1..12}
do
  curl "https://api.nytimes.com/svc/archive/v1/2022/${month}.json?api-key=${API_KEY}" | jq '[.response.docs[] | select(.section_name == "Business Day")]' > "archive/2022/business/2022-${month}.json"
  sleep 12
done
